...
import { Event, EventSchemaClient } from '@sis/types';
import { CIRCULAR_INPUT_FIELDS as inputFields } from '@constants';
...
export const EventForm = () => {
    const methods = useZodForm(EventSchemaClient);
    const {
        register,
        handleSubmit,
        formState: { errors },
    ...
    const saveData = (data: Event) => {
        addEventMutation.mutate({ eventData: data });
                    ...
                    >
                        {inputFields.map((input) => (
                            <InputField
                                ...
                                register={register}
                                control={control}
                                error={errors[input.name as keyof Event]?.message as string}
                            />
                        ))}
